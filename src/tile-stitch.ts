...
yargs
 ...
 .command('$0 <minlat> <minlon> <maxlat> <maxlon> <zoom> <url>', 'Stitch tiles', () => {}, (argv) => {
   stitch(argv.o as string, argv.minlat as number, argv.minlon as number, argv.maxlat as number, argv.maxlon as number, argv.zoom as number, argv.url as string, argv.tilesize as number)
 })
 .argv;
...
async function stitch(filename: string, minlat: number, minlon: number, maxlat: number, maxlon: number, zoom: number, url: string, tilesize: number) {
  ...
  .process(async ([tx, ty]) => {
			...
			const fetchUrl = url.split('{z}').join(zoom + '').split('{x}').join(tx + '').split('{y}').join(ty + '');
	...
	try {
		const tmpBuffer = await sharp({create: {
			width: newWidth,
			height: newHeight,
			channels: 4,
			background: 'rgba(0, 0, 0, 0)'
		}, limitInputPixels: false, sequentialRead: true, failOnError: false})
		.composite(images)
		.toBuffer();
